                                    ...
                                    delegate:(id<ScopeRendererDelegate>)delegate
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(scopeViewDidLiveResize:) name:kScopeViewDidLiveResizeNotification object:nil];
...
- (id<MTLTexture>)loadTextureResource:(NSString*)textureName device:(id<MTLDevice>)device error:(NSError**)error
{
...
- (void)loadMetalWithView:(nonnull MTKView*)view
{
    ...
    // pipelineStateDescriptor.vertexFunction = [defaultLibrary
    // newFunctionWithName:@"scopeInstanceShader"];
    ...
    // Set up pipeline for rendering the frequencies to the offscreen texture.
    {
        ...
        NSAssert(_scopeState,
                 @"Failed to create pipeline state to render the frequencies to a "
                 @"texture: %@",
                 error);
    }
    ...
    // Set up a render pass descriptor for the render pass to render into
    // _frequenciesComposeTargetTexture.
    ...
    _overlayTexture = [self loadTextureResource:@"LargeRastaPattern" device:_device error:&error];
    ...
    _linesUniformBuffer = [_device newBufferWithLength:linesBufferSize options:MTLResourceStorageModeShared];
   ...
   */
    ...
    _frequencyUniformBuffer = [_device newBufferWithLength:frequencyBufferSize options:MTLResourceStorageModeShared];
...
- (void)_updateEngine
{
    ...
    // matrix_float4x4 feedbackMatrix = matrix4x4_rotation(M_PI_2 + _rotation,
    // rotationAxis);
    ...
    // This may not be a great strategy from a scientific angle - it is a
    ...
    size_t samplesToGo = [self.visual.sample rawSampleFromFrameOffset:f frames:kWindowSamples outputs:sourceChannels];
    ...
    while (bestPositiveStreakLength == 0) {
        ...
        unsigned long long framesReceived = [self->_visual.sample rawSampleFromFrameOffset:f frames:self->_sampleCount outputs:sourceChannels];
        ...
        for (size_t i = 0; i < framesToGo; i++) {
            ...
            if (!triggered) {
                ...
                if (f >= self->_minTriggerOffset &&  // Prevent triggering before a
                                                     // minimum offset.
                    (data > 0.0f && last <= 0.0f)) {
                    zeroCrossingOffset = f;
                    positiveStreakLength = 0;
    ...
    // We are exploiting the traversal over the displayed samples by adding the
    // level meter feeding.
...
{
    // Lets do our stuff in the render queue so we dont need to worry about
    // concurrent access.
    dispatch_async(_renderQueue, ^{
        // FIXME: All of the screen resizeing and aspect ratio calculation seems
        // fishy - needs a do-over I fear.
        ...
        // An FFT line including the gap/s is the total width of the screen devided
        // by the number of FFT spectrum buckets.
        ...
        CGSize erodeSize =
            NSMakeSize((((ceil(ScaleWithOriginalFrame(erodePoints, self->_defaultSize.height, self->_defaultSize.height + (size.height / 30)))) / 2) * 2) + 1,
                       (((ceil(ScaleWithOriginalFrame(erodePoints, self->_defaultSize.height, self->_defaultSize.height + (size.height / 30)))) / 2) * 2) + 1);
        CGSize frequencyErodeSize = NSMakeSize(
            (((ceil(ScaleWithOriginalFrame(frequencyErodePoints, self->_defaultSize.height, self->_defaultSize.height + (size.height / 30)))) / 2) * 2) + 1,
            0.0);
        CGSize bloomSize = NSMakeSize(
            ((((int) ceil(ScaleWithOriginalFrame(bloomPoints, self->_defaultSize.height, self->_defaultSize.height + (size.height / 30)))) / 2) * 2) + 1,
            ((((int) ceil(ScaleWithOriginalFrame(bloomPoints, self->_defaultSize.height, self->_defaultSize.height + (size.height / 30)))) / 2) * 2) + 1);
        CGSize frequencyBloomSize = NSMakeSize(
            ((((int) ceil(ScaleWithOriginalFrame(bloomPoints, self->_defaultSize.height, self->_defaultSize.height + (size.height / 30)))) / 2) * 2) + 1, 1.0);
        self->_blur = [[MPSImageGaussianBlur alloc] initWithDevice:view.device sigma:sigma];
        self->_blur.edgeMode = MPSImageEdgeModeClamp;
        self->_erode = [[MPSImageAreaMin alloc] initWithDevice:view.device kernelWidth:erodeSize.width kernelHeight:erodeSize.height];
        ...
        self->_bloom = [[MPSImageBox alloc] initWithDevice:view.device kernelWidth:bloomSize.width kernelHeight:bloomSize.height];
        self->_frequencyBloom = [[MPSImageBox alloc] initWithDevice:view.device kernelWidth:frequencyBloomSize.width kernelHeight:frequencyBloomSize.height];
...
- (void)_renderScope:(id<MTLCommandBuffer>)commandBuffer
{
    ...
    [renderEncoder setVertexBuffer:_linesUniformBuffer offset:_linesBufferOffset atIndex:BufferIndexScopeLines];
    ...
    [_bloom encodeToCommandBuffer:commandBuffer sourceTexture:_scopeTargetTexture destinationTexture:_bufferTexture];
}
...
- (void)_renderFrequencies:(id<MTLCommandBuffer>)commandBuffer
{
    ...
    [renderEncoder setVertexBuffer:_frequencyUniformBuffer offset:_frequencyBufferOffset atIndex:BufferIndexFrequencies];
...
- (void)drawInMTKView:(nonnull MTKView*)view
{
    dispatch_barrier_sync(_renderQueue, ^{
        ...
        [_bloom encodeToCommandBuffer:commandBuffer sourceTexture:_composeTargetTexture destinationTexture:_bufferTexture];
        [_erode encodeToCommandBuffer:commandBuffer sourceTexture:_bufferTexture destinationTexture:_lastTexture];

        [self _renderFrequenciesCompose:commandBuffer];

        [_frequencyBloom encodeToCommandBuffer:commandBuffer sourceTexture:_frequenciesComposeTargetTexture destinationTexture:_composeTargetTexture];

        //        [_blur encodeToCommandBuffer:commandBuffer
        //                       sourceTexture:_composeTargetTexture
        //                  destinationTexture:_bufferTexture];

        [_frequencyErode encodeToCommandBuffer:commandBuffer sourceTexture:_composeTargetTexture destinationTexture:_lastFrequenciesTexture];
