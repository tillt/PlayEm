...
- (void)renderWithDelta:(CFTimeInterval)dt
{
    ...
    CGPoint headPos = CGPointMake(insetCenter.x + r * 0.85 * cos(_angle), insetCenter.y + r * 0.85 * sin(_angle));
    ...
    CIFilter* head = [CIFilter
        filterWithName:@"CIRadialGradient"
         keysAndValues:@"inputCenter", [CIVector vectorWithX:headPosCI.x Y:headPosCI.y], @"inputRadius0", @(0.0), @"inputRadius1", @(dotRadius), @"inputColor0",
                       beamCI, @"inputColor1", [[CIColor alloc] initWithRed:beamCI.red green:beamCI.green blue:beamCI.blue alpha:0.0], nil];
    CGRect dotCrop = CGRectMake(headPosCI.x - (dotRadius + 3.0), headPosCI.y - (dotRadius + 3.0), (dotRadius + 3.0) * 2.0, (dotRadius + 3.0) * 2.0);
    ...
    CIFilter* headSpotGradient = [CIFilter
        filterWithName:@"CIRadialGradient"
         keysAndValues:@"inputCenter", [CIVector vectorWithX:headPosCI.x Y:headPosCI.y], @"inputRadius0", @(headSpotSize), @"inputRadius1", @(headSpotSize),
                       @"inputColor0", [[CIColor alloc] initWithCGColor:beam], @"inputColor1", [[CIColor alloc] initWithCGColor:beam], nil];
    CGRect headRect = CGRectMake(headPosCI.x - headSpotSize * 0.5, headPosCI.y - headSpotSize * 0.5, headSpotSize, headSpotSize);
