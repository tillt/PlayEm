...
+ (CIFilter*)sharedBloomFilter
{
    ...
    dispatch_once(&once, ^{
        ...
        [sharedInstance setValue:[NSNumber numberWithFloat:1.0] forKey:@"inputIntensity"];
    });
        ...
        _viewConfiguration = @{
            kInfoPageKeyDetails : @{
                ...
                @"rating" : @{
                    ...
                    @"values" : [[[MediaMetaData starsQuantums] allValues] sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)],
                },
                @"tempo" : @{
                    @"order" : @12,
                    @"width" : @40,
...
- (void)loadControlsWithView:(NSView*)view pageKey:(NSString*)pageKey
{
    ...
    for (NSString* key in reversed) {
        ...
        textField.frame = NSMakeRect(kBorderWidth, y - floor((rowUnitHeight - [[Defaults sharedDefaults] normalFontSize]) / 2.0f), nameFieldWidth,
        ...
        case InfoControlTypeText: {
            ...
            textField.frame = NSMakeRect(x, y - floor(editable ? 0.0f : (rowUnitHeight - [[Defaults sharedDefaults] normalFontSize]) / 2.0f),
                                         width - kBorderWidth, (rows * rowUnitHeight) + kRowInset);
            [view addSubview:textField];
        ...
        case InfoControlTypeCombo: {
            ...
            comboBox.frame = NSMakeRect(x, y, width - kBorderWidth, rowUnitHeight + kRowInset);
        ...
        case InfoControlTypePopup: {
            ...
            popup.frame = NSMakeRect(x, y, width - kBorderWidth, rowUnitHeight + kRowInset);
        ...
        case InfoControlTypeCheck: {
            ...
            NSButton* button = [NSButton checkboxWithTitle:title target:self action:selector];
            button.frame = NSMakeRect(x, y, width - kBorderWidth, rowUnitHeight + kRowInset);
        ...
        // 2. The symbol is line based -- thus for lines with multiple entries (ie
        // track / tracks) it needs to track multiple value fields.
        ...
        if (extra != nil) {
            ...
            textField.frame = NSMakeRect(x + width, y - floor((rowUnitHeight - [[Defaults sharedDefaults] normalFontSize]) / 2.0f), dynamicWidth,
                                         (rows * rowUnitHeight) + kRowInset);
            [view addSubview:textField];
            ...
            textField.frame = NSMakeRect(x + width + kBorderWidth + dynamicWidth,
                                         y - floor(editable ? 0.0f : (rowUnitHeight - [[Defaults sharedDefaults] normalFontSize]) / 2.0f), width - kBorderWidth,
                                         (rows * rowUnitHeight) + kRowInset);
            [view addSubview:textField];
            ...
            checkMark.frame = NSMakeRect(x + width + kBorderWidth + width + dynamicWidth - 5.0, y, 20.0, rowUnitHeight + kRowInset);
            dict[extra[@"key"]] = extraElements;
        } else {
            checkMark.frame = NSMakeRect(x + width - 5.0, y, 20.0, rowUnitHeight + kRowInset);
        }
...
- (void)loadArtworkWithView:(NSView*)view
{
    ...
    // We initialize with that "UnknownSong" image but later we will get
    // overwritten also by metadata without artwork - causing an empty view - by
    // design to be extra clear in the message to the user: there is no artwork
    // available.
    _largeCoverView.image = [NSImage resizedImageWithData:[MediaMetaData defaultArtworkData] size:NSMakeSize(imageWidth, imageWidth)];
    ...
    _largeCoverView.frame = CGRectMake((self.view.bounds.size.width - (imageWidth + (2 * kViewLeftMargin))) / 2.0f, kViewTopMargin, imageWidth, imageWidth);
    ...
    NSArray* dragTypes = [NSArray arrayWithObjects:NSCreateFileContentsPboardType(@"jpeg"), NSCreateFileContentsPboardType(@"jpg"),
                                                   NSCreateFileContentsPboardType(@"tiff"), NSCreateFileContentsPboardType(@"png"), nil];
...
- (void)loadLyricsWithView:(NSView*)view
{
    ...
    scrollView.frame = CGRectMake(kViewLeftMargin * 2, kViewTopMargin + 10.0, self.view.bounds.size.width - 60.0f, self.view.bounds.size.height - 230.0f);
...
- (void)loadView
{
    ...
    //    NSVisualEffectView* view = [[NSVisualEffectView alloc]
    //    initWithFrame:NSMakeRect(0.0, 0.0, self.preferredContentSize.width,
    //    self.preferredContentSize.height)]; view.material =
    //    NSVisualEffectMaterialTitlebar; view.autoresizingMask =
    ...
    _effectBelowHeader = [[NSVisualEffectView alloc]
        initWithFrame:NSMakeRect(0, self.preferredContentSize.height - (headerHeight - 30.0f), self.preferredContentSize.width, headerHeight)];
    ...
    _progress = [[NSProgressIndicator alloc] initWithFrame:NSMakeRect((_effectBelowHeader.frame.size.width - progressIndicatorWidth) / 2.0,
                                                                      ...
                                                                      progressIndicatorWidth, progressIndicatorHeight)];
    ...
    _smallCoverView.frame =
        CGRectMake(kViewLeftMargin + kViewLeftMargin, _effectBelowHeader.frame.size.height - (imageWidth + kViewTopMargin), imageWidth, imageWidth);
    ...
    textField.alignment = NSTextAlignmentLeft;
    textField.frame = NSMakeRect(x, y, fieldWidth, [[Defaults sharedDefaults] bigFontSize] + kRowInset);
    ...
    textField.selectable = NO;
    textField.alignment = NSTextAlignmentLeft;
    textField.frame = NSMakeRect(x, y, fieldWidth, [[Defaults sharedDefaults] normalFontSize] + kRowInset);
    [_effectBelowHeader addSubview:textField];
...
- (void)viewWillAppear
{
    ...
    segment.frame = NSMakeRect(kViewLeftMargin * 4, self.view.frame.size.height - (100.0 + (kViewTopMargin * 2)),
                               self.view.bounds.size.width - (kViewLeftMargin * 8), 30.0);
    [self.view addSubview:segment];
    [segment setSelectedSegment:0];

    _tabView = [[NSTabView alloc] initWithFrame:NSMakeRect(kViewLeftMargin, kViewTopMargin * 2, self.view.frame.size.width,
    ...
    // For some weird reason the NSColour windowBack.. etc colours do not seem to
    // be actual colours - but just transparent. So we hardcode something here for
    // now.
    //
    ...
    // the same when showing the info from a library entry - it reads up the
    // latest info from the song file metadata.
...
- (void)updateControlsFromMetas
{
    ...
    // Identify any meta attribute that is common / not common in the given list
    // of metas. A common meta attribute is one that is universally equally set
    // for all metas in our list.

    // `deltaKeys` is a map of keys to booleans, indicating a meta attribute
    // diverting from other metas in the list.
    ...
    // Now that we know all commons and deltas among the metas, we can show it in
    // the header.
    ...
    for (NSString* pageKey in pageKeys) {
        ...
        for (NSString* key in keys) {
            ...
            if (![pageKey isEqualToString:kInfoPageKeyDetails] || [deltaKeys objectForKey:key] == nil) {
                // The meta data in question is common - it did not change while
                // editing.
                NSString* value = @"";
                ...
                if (placeHolder != nil && [control respondsToSelector:@selector(cell)]) {
                    ...
                    cell.placeholderAttributedString = [[NSAttributedString alloc] initWithString:placeHolder attributes:attrs];
                }
            }
        }
    }
...
- (void)compilationAction:(id)sender
{
    ...
    // Even if we signalled allowing mixed state, the user decided and this we
...
- (void)controlTextDidChange:(NSNotification*)notification
{
    ...
    //        }
    //    }
    //    NSAssert(key != nil, @"couldnt find the key for the control that
