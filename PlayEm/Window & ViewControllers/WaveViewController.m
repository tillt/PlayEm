...
- (id)init
{
    ...
    if (self) {
        ...
        _handleColors = @[
            [NSColor tertiaryLabelColor], [[Defaults sharedDefaults] regularFakeBeamColor], [[Defaults sharedDefaults] lightFakeBeamColor],
...
- (void)setFrames:(unsigned long long)frames
{
    ...
    // As the total amount of frames for this sample has changed, we will also
    // need to reload pretty much everything -- the beat layer is skipped here as
    // it is highly likely not
...
- (void)loadBeatsForTile:(TileView*)tile
{
    ...
    if (currentBeatIndex == ULONG_LONG_MAX) {
        // NSLog(@"beat buffer from frame %lld (screen %f) not yet available",
        // frameOffset, start);
        return;
    }
...
- (NSAttributedString*)textWithFont:(NSFont*)font color:(NSColor*)color width:(CGFloat)width text:(NSString*)text
{
    ...
    NSDictionary* attributes = @{NSForegroundColorAttributeName : color, NSFontAttributeName : font, NSParagraphStyleAttributeName : style};
...
- (CALayer*)trackLayer
{
    ...
    background.colors = @[
        (id) [[[Defaults sharedDefaults] regularFakeBeamColor] colorWithAlphaComponent:0.4].CGColor, (id)[NSColor clearColor].CGColor,
    ...
    // background.borderColor = [[Defaults sharedDefaults]
    // lightBeamColor].CGColor; background.borderWidth = 0.5f;
    ...
    // imageLayer.compositingFilter = [CIFilter
    // filterWithName:@"CILinearDodgeBlendMode"];
    ...
    // artistLayer.constraints = @[ minXConstraint, minYConstraint,
    // maxXConstraint, heightConstraint];
    ...
    //    shinyLayer.backgroundColor = [[Defaults sharedDefaults]
    //    lightFakeBeamColor].CGColor; shinyLayer.rasterizationScale =
    //    rootLayer.contentsScale; shinyLayer.contentsScale = [[NSScreen
    //    mainScreen] backingScaleFactor]; shinyLayer.masksToBounds = YES;
    //    shinyLayer.name = @"WaveMarkDescriptionShinyLayer";
    //    shinyLayer.frame = CGRectMake(1.0f, 5.0f, 3.0f, backgroundSize.height
    //    - 10.0f); [background addSublayer:shinyLayer];
...
- (void)updateTrackDescriptions
{
    ...
    //
    // See if we already have sublayers that cover these needed track frame
    // offsets.
    //
    ...
    for (CALayer* layer in layers) {
        ...
        if (![neededTrackFrames containsObject:keyNumber]) {
            ...
            // Take this track frame offset off the to-do list - we already got it
            // covered.
...
//        }];
        ...
        NSAttributedString* artist = [self textWithFont:_artistFont color:_artistColor text:track.meta.artist != nil ? track.meta.artist : @""];
        artistLayer.string = artist;

        NSAttributedString* time = [self textWithFont:_timeFont color:_timeColor text:[_delegate stringFromFrame:[track.frame unsignedLongLongValue]]];
    ...
    if (trackFrames.count > 0) {
        for (size_t i = 0; i < trackFrames.count; i++) {
            ...
            // We position using the layout manager to allow for constraint based
            // layout.
            ...
            if (next != nil) {
                maxXConstraint = [CAConstraint constraintWithAttribute:kCAConstraintMaxX
                                                            relativeTo:[next stringValue]
                                                             attribute:kCAConstraintMinX
                                                                offset:-2.0f];
            } else {
            ...
            // currentLayer.sublayers[kClippingHostLayerIndex].sublayers[kImageLayerIndex].filters
            // = @[ _vibranceFilter ];
        }
...
- (void)updateTiles
{
    ...
    // Figure out the tile frames we would need to get full coverage and add them
    // to the to-do list.
...
// FIXME: Assert we cant solve this better using some variant of a CAShapeLayer.
// Right now this custom layer drawing is hooked into the screen asking for
// refresh.
// FIXME: If we used a CAShapeLayer d that way we would have a lot more control
// over the process as we determine what to put onto the layer the moment we
// composite the screen and not the moment the screen asks for our refresh.
- (void)drawLayer:(CALayer*)layer inContext:(CGContextRef)context
{
   ...
   */
    ...
    if (buffer == nil) {
        // We didnt find any visual data - lets make sure we get some in the near
        // future...
...
- (void)dragMark:(NSPoint)location
{
    ...
    _draggingLayer.constraints = @[ minXConstraint, _draggingLayer.constraints[1], _draggingLayer.constraints[2], _draggingLayer.constraints[3] ];
    ...
    NSAttributedString* time = [self textWithFont:timeFont color:timeColor text:[_delegate stringFromFrame:newFrame]];
    timeLayer.string = time;
...
///
/// We need to make sure nothing changes the cursor in our view - thus we
/// override the handler but do not pass down the event. That way our cursor can
/// be controlled exclusively during the mouseMove handler.
///
/// We cant use this handler as it appears to get called only sporadically,
/// depending on other screen activities -- entirely enigmatic but the result is
/// only moveMove gets reliable called and that is where we do our cursor
/// updates.
///
/// Without this override, some other stuff trashes our cursor updates -- i have
...
- (void)duckStackAfter:(CALayer*)popularMarker
{
    ...
    for (CALayer* layer in orderedLayers) {
        ...
        layer.constraints = @[ minXConstraint, layer.constraints[1], layer.constraints[2], layer.constraints[3] ];
        x += 20.0;
    }
    [wv.markLayer setNeedsLayout];
    [wv.markLayer layoutIfNeeded];
...
- (void)mouseMoved:(NSEvent*)event
{
    ...
    if (marker == nil) {
        // We now didnt find any marker under the cursor - lets see if we are on any
        // background.
    ...
    if (markerBackgroundLayer != nil) {
        // Highlight by making the image layer entirely opaque,unless we are on top
        // of a marker.
        markerBackgroundLayer.sublayers[kClippingHostLayerIndex].sublayers[kImageLayerIndex].opacity = marker == nil ? 1.0f : 0.0f;
        // markerBackgroundLayer.sublayers[kClippingHostLayerIndex].sublayers[kImageLayerIndex].filters
        // = nil;
...
- (void)mouseUp:(NSEvent*)event
{
    ...
    // Fake a mouse move event to assert that the cursor gets recognized above the
    // handle right away.
...
- (void)updateHeadPosition
{
    ...
    if (self.view.enclosingScrollView != nil) {
        if (_followTime) {
            CGPoint pointVisible =
                CGPointMake(self.view.enclosingScrollView.bounds.origin.x + floor(head - (self.view.enclosingScrollView.bounds.size.width / 2.0)),
            ...
            // If the user has just requested some scrolling, do not interfere but
            // wait as long as that state is up.
            if (!_userMomentum) {
                // If the head came back into the middle of the screen, snap back to
                // following time with the scrollview.
                const CGFloat delta = 1.0f;
                CGFloat visibleCenter =
                    self.view.enclosingScrollView.documentVisibleRect.origin.x + (self.view.enclosingScrollView.documentVisibleRect.size.width / 2.0f);
                if (visibleCenter - delta <= head && visibleCenter + delta >= head) {
                    _followTime = YES;
                }
            }
        }
        // There is no real scrolling for the track descriptions, we fake that by
        // continuously updating the layer positions.
        [self updateTrackDescriptions];
    }
