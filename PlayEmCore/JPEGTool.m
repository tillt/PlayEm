...
+ (CVPixelBufferRef)create420PixelBuffer:(CGSize)size
{
    ...
    if (CVPixelBufferCreate(kCFAllocatorDefault, size.width, size.height, kCVPixelFormatType_420YpCbCr8BiPlanarFullRange, (__bridge CFDictionaryRef) attrs,
                            &pb) != kCVReturnSuccess) {
        return NULL;
    }
    return pb;
...
+ (void)renderNSImage:(NSImage*)image intoPixelBuffer:(CVPixelBufferRef)pb
{
    ...
    [ctx render:ciImage toCVPixelBuffer:pb bounds:CGRectMake(0, 0, CVPixelBufferGetWidth(pb), CVPixelBufferGetHeight(pb)) colorSpace:cs];
...
static void JPEGCallback(void* outputCallbackRefCon, void* sourceFrameRefCon, OSStatus status, VTEncodeInfoFlags infoFlags, CMSampleBufferRef sampleBuffer)
{
...
+ (nullable NSData*)encodeImageToJPEG420:(NSImage*)image quality:(CGFloat)quality
{
    ...
    OSStatus err = VTCompressionSessionCreate(kCFAllocatorDefault, image.size.width, image.size.height, kCMVideoCodecType_JPEG, NULL, NULL, NULL, JPEGCallback,
                                              &ctx, &session);
    ...
    VTSessionSetProperty(session, kVTCompressionPropertyKey_Quality, (__bridge CFTypeRef) @(quality));
...
+ (BOOL)isJPEGYUV420:(NSData* _Nonnull)jpegData samplingInfo:(JPEGSamplingInfo* _Nullable)infoOut
{
    ...
    while (i + 3 < len) {
        ...
        if (IsSOFMarker(marker) && segLen >= 2 + 6 + 3 * 3) {
            ...
            localInfo.hv[0][0] = hv1 >> 4;
            localInfo.hv[0][1] = hv1 & 0x0F;
            localInfo.hv[1][0] = hv2 >> 4;
            localInfo.hv[1][1] = hv2 & 0x0F;
            localInfo.hv[2][0] = hv3 >> 4;
            localInfo.hv[2][1] = hv3 & 0x0F;
            localInfo.isYUV420 = (localInfo.hv[0][0] == 2 && localInfo.hv[0][1] == 2 && localInfo.hv[1][0] == 1 && localInfo.hv[1][1] == 1 &&
