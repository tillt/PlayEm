...
- (int)readFromMP3FileWithError:(NSError**)error
{
    ...
    if (tag != NULL) {
        // Process the taglib standard stuff - an ID3 v1 data subset by its
        // selection.
        [self updateWithKey:@"title" string:[NSString stringWithCString:taglib_tag_title(tag) encoding:NSUTF8StringEncoding]];
        [self updateWithKey:@"artist" string:[NSString stringWithCString:taglib_tag_artist(tag) encoding:NSUTF8StringEncoding]];
        [self updateWithKey:@"album" string:[NSString stringWithCString:taglib_tag_album(tag) encoding:NSUTF8StringEncoding]];
        [self updateWithKey:@"year" string:[[NSNumber numberWithUnsignedInt:taglib_tag_year(tag)] stringValue]];
        [self updateWithKey:@"comment" string:[NSString stringWithCString:taglib_tag_comment(tag) encoding:NSUTF8StringEncoding]];
        [self updateWithKey:@"track" string:[[NSNumber numberWithUnsignedInt:taglib_tag_track(tag)] stringValue]];
        [self updateWithKey:@"genre" string:[NSString stringWithCString:taglib_tag_genre(tag) encoding:NSUTF8StringEncoding]];
    }
    ...
    if (propertiesMap != NULL) {
        ...
        while (*keyPtr) {
            ...
            while (valPtr && *valPtr) {
                NSString* key = [NSString stringWithCString:*keyPtr encoding:NSUTF8StringEncoding];
                NSString* values = [NSString stringWithCString:*valPtr encoding:NSUTF8StringEncoding];
    ...
    if (properties != NULL) {
        ...
        self.format = @"MPEG audio file";  // FIXME: This is wrong - AIFF and WAV
                                           // could also involved.
    } else {
...
// Note that we are avoiding using `BOOL` in the signature here as that gets
// defined as `int` by "taglib_c.h". Trouble is, Objective C typedefs BOOL to
// various types, depending on the platform and processor architecture. See
    ...
    for (NSString* key in [tagMap allKeys]) {
       ...
       */
            if (self.artwork != nil) {
                ...
                TAGLIB_COMPLEX_PROPERTY_PICTURE(props, self.artwork.bytes, (unsigned int) self.artwork.length, "", m, "Front Cover");
                NSLog(@"setting complex tag: \"%@\" = %p", key, self.artwork);
                taglib_complex_property_set(file, [key cStringUsingEncoding:NSUTF8StringEncoding], props);
            } else {
                taglib_complex_property_set(file, [key cStringUsingEncoding:NSUTF8StringEncoding], NULL);
            }
        } else {
            ...
            // NOTE: We are possible reducing the accuracy of a DATE as we will only
            // store the year while the original may have had day and month included.
            NSLog(@"setting tag: \"%@\" = \"%@\"", key, value);
            taglib_property_set(file, [key cStringUsingEncoding:NSUTF8StringEncoding],
