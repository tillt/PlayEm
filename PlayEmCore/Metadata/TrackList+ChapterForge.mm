                      ...
                      std::vector<ChapterImageSample>& imageChapters, FrameToSeconds frameToSeconds)
{
    ...
    for (NSNumber* value in frames) {
        ...
        // Title
        //
        // We encode the title and the artist into the first chapter text track to
        // make things look beautiful and informative in players like VLC, QuickTime
        // and alike.
        NSString* text = nil;
        ...
        // URL & Artist
        //
        // We encode artist here again so that we can later split it out from the
        // title track when parsing the file again. This is done to assert that even
        // titles or artists containing dashes would not confuse the parser when
        // attemptng to split.
        //
        // The artist encoding is quirky to say it mildly. We are exploiting the
        // fact that the URL track, in the end is not parsed by any player we know
        // of. AVFoundation only ever surfaces the URL if it gets referenced by the
        // title track, making the entire existence of the URL track questionable.
        // However, that is what Apple and other players get along with and this is
        // where we piggyback on to stay compatible.
        //
        // Note that this schema appears to scale further - if we ever needed to
        // encode more timed information, we could simply add another track - it
        // appears sane players will ignore.
        ...
        // this might be a bad idea as it results in the previous track image
        // getting shown again.
    ...
    while ((sb = [trackOut copyNextSampleBuffer]) != NULL) {
        ...
        if (bb) {
            ...
            if (len > 2) {
                ...
                // tx3g: first 2 bytes = big-endian text length, then UTF-8 text, then
                // optional boxes (e.g., href)
                uint16_t beLen = 0;
