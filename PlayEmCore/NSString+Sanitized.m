...
static NSString *cf_hexUTF8(NSString *s) {
    ...
    for (NSUInteger i = 0; i < utf8.length; i++) {
        ...
        if (i + 1 < utf8.length) [hex appendString:@" "];
    }
    return hex;
}
...
static NSInteger cf_mojibakeScore(NSString* s)
{
    ...
    [s enumerateSubstringsInRange:NSMakeRange(0, len)
        ...
        if ([markers characterIsMember:[substring characterAtIndex:0]]) {
            ...
            bad += 2; // heavier penalty to steer scoring away from mojibake like è
        }
    }];
...
// normalize -> replacements -> (if markers remain) strip markers -> replacements -> normalize.
                ...
                for (NSString* cur in frontier) {
                    ...
                    for (NSNumber* encNum in encs) {
                        ...
                        if (mis && ![mis isEqualToString:orig]) {
                            if (!m[mis]) m[mis] = [NSMutableSet set];
    ...
    [map enumerateKeysAndObjectsUsingBlock:^(NSString* key, NSArray<NSString*>* vals, BOOL* stop) {
        ...
        for (NSString* val in vals) {
            ...
            if (score < currentScore ||
                (score == currentScore && candPriority < bestPriority) ||
...
- (NSString*)sanitizedMetadataString
{
    ...
    // Build candidates: base (with replacements), Latin1->UTF8, MacRoman->UTF8, and legacy recodes.
    ...
    if (latin1) [candidates addObject:cf_applyReplacements(latin1)];

    NSString* mac = cf_reinterpret(s, NSMacOSRomanStringEncoding, NSUTF8StringEncoding);
    if (mac) [candidates addObject:cf_applyReplacements(mac)];
    ...
    // Pass 2: if markers remain, strip them, then re-apply replacements and normalize.
    if (hasBadMarkers(s)) {
