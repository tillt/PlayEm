...
{
    // NSLog(@"rounding BPM - min: %.4f, center: %.4f, max: %.4f", minBpm,
    // centerBpm, maxBpm);
...
/**
 ...
 has a phase error that remains below a threshold. With the given detection
 method, we hardly ever get to 16 beats of stable detection - even with enabled
 lowpass filter.

 This is mostly a copy of code from MixxxDJ.
 from
 https://github.com/mixxxdj/mixxx/blob/8354c8e0f57a635acb7f4b3cc16b9745dc83312c/src/track/beatutils.cpp#L51
 */
- (NSData* _Nullable)retrieveConstantRegions
{
    NSLog(@"pass two: locate constant regions");

    // Original comment doesnt apply exactly -- we are not using the QM detector
    // but the libaubio one. Anyway, the rest applies equally.
    //---
    // The aubio detector has a step size of 256 frames @ 44100 Hz. This means
    // that Single beats have has a jitter of +- 6 ms around the actual position.
    // Expressed in BPM it means we have for instance steps of these BPM value
    // around 120 BPM 117.454 - 120.185 - 123.046 - 126.048 A pure electronic
    // 120.000 BPM track will have many 120,185 BPM beats and a few 117,454 BPM
    // beats to adjust the collected offset. This function irons these adjustment
    // beats by adjusting every beat to the average of a likely constant region.
    // Therefore we loop through the coarse beats and calculate the average beat
    // length from the first beat.
    // A inner loop checks for outliers using the momentary average as beat
    // length. Once we have found an average with only single outliers, we store
    // the beats using the current average to adjust them by up to +-6 ms. Then we
    // start with the region from the found beat to the end.
    //---

    const double maxPhaseError = kMaxSecsPhaseError * self.sample.renderedSampleRate;
    const double maxPhaseErrorSum = kMaxSecsPhaseErrorSum * self.sample.renderedSampleRate;
    ...
    while (leftIndex < coarseBeatCount - 1) {
        ...
        for (; i <= rightIndex; ++i) {
            ...
            if (fabs(phaseError) > maxPhaseError) {
                outliersCount++;
                // The first beat must not be an outlier just like the number of
                // outliers overall must not be beyond
        ...
        if (i > rightIndex) {
            ...
            // Verify that the first and the last beat are not correction beats in the
            // same direction as this would bend meanBeatLength unfavorably away from
            // the optimum.
            ...
            //            else {
            //                NSLog(@"mean border error got too large for beat %ld to
            //                %ld = %f", leftIndex, rightIndex, regionBorderError);
            //            }
        }
...
/**
 This is mostly a copy of code from MixxxDJ.
 from
 https://github.com/mixxxdj/mixxx/blob/8354c8e0f57a635acb7f4b3cc16b9745dc83312c/src/track/beatutils.cpp#L140

 There is however a slight change that has rather dramatic consequences in my
 testing. Any functional modifications are highlighted by comments lead by
 `CHANGE:`.
 */
- (double)makeConstBpm:(NSData*)constantRegions firstBeat:(signed long long*)pFirstBeat
{
    ...
    // In these regions the detected beat might is floating around and is just
    // wrong. The track may also has regions with different rhythm giving
    // instruments. They have a different shape of onsets and introduce a static
    // beat offset. The track may also have break beats or other issues that makes
    // the detector hook onto a beat that is by an integer fraction off the
    // original metronome.
    ...
    for (int i = 0; i < regionsCount - 1; ++i) {
        ...
        // CHANGE: Calculate the number of beats right away instead of relying on
        // the region length for comparison. The observation was that the old code
        // did prefer long regions with long beat duration which in my opinion it
        // should not. The updated code shows a much better resultset in that we
        // later have a higher success rate in identifying additional constant
        // regions that fit in phase.
    ...
    double longestRegionBeatLengthMin = longestRegionBeatLength - ((kMaxSecsPhaseError * self.sample.renderedSampleRate) / longestRegionNumberOfBeats);
    double longestRegionBeatLengthMax = longestRegionBeatLength + ((kMaxSecsPhaseError * self.sample.renderedSampleRate) / longestRegionNumberOfBeats);

    int startRegionIndex = midRegionIndex;

    NSLog(@"pass four: find a region at the beginning of the track with a "
          @"similar tempo and phase");
    ...
    for (int i = 0; i < midRegionIndex; ++i) {
        ...
        const double thisRegionBeatLengthMin = regions[i].beatLength - ((kMaxSecsPhaseError * self.sample.renderedSampleRate) / numberOfBeats);
        const double thisRegionBeatLengthMax = regions[i].beatLength + ((kMaxSecsPhaseError * self.sample.renderedSampleRate) / numberOfBeats);
        // Check if the tempo of the longest region is part of the rounding range of
        // this region.
        if (longestRegionBeatLength > thisRegionBeatLengthMin && longestRegionBeatLength < thisRegionBeatLengthMax) {
            ...
            if (minNumberOfBeats != maxNumberOfBeats) {
                ...
                // NSLog(@"ambiguous number of beats - %d != %d - find a closer region
                // ...", minNumberOfBeats, maxNumberOfBeats);
                continue;
            }
            ...
            if (newBeatLength > longestRegionBeatLengthMin && newBeatLength < longestRegionBeatLengthMax) {
                ...
                longestRegionBeatLengthMin = longestRegionBeatLength - ((kMaxSecsPhaseError * self.sample.renderedSampleRate) / longestRegionNumberOfBeats);
                longestRegionBeatLengthMax = longestRegionBeatLength + ((kMaxSecsPhaseError * self.sample.renderedSampleRate) / longestRegionNumberOfBeats);
                startRegionIndex = i;
                break;
            }
        }
    ...
    NSLog(@"pass five: find a region at the end of the track with a similar "
          @"tempo and phase");
    ...
    NSLog(@"first beat: %lld, longest region length: %.2f, number of beats: %d", regions[startRegionIndex].firstBeatFrame, longestRegionLength,
          longestRegionNumberOfBeats);

    NSLog(@"pass six: create a const region from the first beat of the first "
          @"region to the last beat of the last region");

    // Create a const region from the first beat of the first region to the last
    // beat of the last region.

    const double minRoundBpm = (double) (60.0 * self.sample.renderedSampleRate / longestRegionBeatLengthMax);
    const double maxRoundBpm = (double) (60.0 * self.sample.renderedSampleRate / longestRegionBeatLengthMin);
    const double centerBpm = (double) (60.0 * self.sample.renderedSampleRate / longestRegionBeatLength);
    ...
    if (pFirstBeat) {
        ...
        //
        // We calculate the frame for the very first beat according to the longest
        // region identified. Now this may not be optimal just yet. There may be
        // cases with some initial silence within the first beats. To account for
        // such cases, we skip numbering beats until the initial silence has passed.
        // Then there may be cases where the true first beat is slightly before the
        // beginning of the song -- weird but there are plenty of examples of such
        // songs. For catching such cases, we check if such misplaced first beat
        // would be less than a quarter beat before the recording.

        const double roundedBeatLength = 60.0 * self.sample.renderedSampleRate / roundBpm;
        ...
        // Evaluate if a possible first bar appears to be a bit beyond the beginning
        // of the sample.
...
/**
 This is mostly a copy of code from MixxxDJ.
 from
 https://github.com/mixxxdj/mixxx/blob/8354c8e0f57a635acb7f4b3cc16b9745dc83312c/src/track/beatutils.cpp#L386
 */
- (unsigned long long)adjustPhase:(unsigned long long)firstBeat bpm:(double)bpm
{
    const double beatLength = 60 * self.sample.renderedSampleRate / bpm;
    ...
    for (int i = 0; i < coarseBeatCount; i++) {
        ...
        if (fabs(offset) < (kMaxSecsPhaseError * self.sample.renderedSampleRate)) {
            offsetAdjust += offset;
            offsetAdjustCount++;
        }
    }
    ...
    // NSAssert(fabs(offsetAdjust) < (kMaxSecsPhaseError * _sample.rate),
    // @"unexpexted phase adjustment");
...
/**
 This is mostly a copy of code from MixxxDJ.
 from
 https://github.com/mixxxdj/mixxx/blob/8354c8e0f57a635acb7f4b3cc16b9745dc83312c/src/track/beatfactory.cpp#L51
 */
- (NSMutableData* _Nullable)makeConstantBeats:(NSData*)constantRegions
{
    ...
    const double beatLength = 60.0 * self.sample.renderedSampleRate / constBPM;
