...
- (void)testImageCacheHitsAndClear
{
    ...
    [[ImageController shared] imageForData:png key:@"test-key" size:20.0 completion:^(NSImage *img) {
        first = img;
        XCTAssertNotNil(img);
...
- (void)testResolveDataForURL
{
    ...
    [[ImageController shared] resolveDataForURL:url callback:^(NSData *data) {
...
- (NSImage *)syncImageForData:(NSData *)data key:(NSString *)key size:(CGFloat)size timeout:(NSTimeInterval)timeout
{
    ...
    [[ImageController shared] imageForData:data key:key size:size completion:^(NSImage *img) {
        result = img;
        [expect fulfill];
    }];
...
- (void)testCacheEvictionWhenCostLimitExceeded
{
    ...
    // Insert a couple more unique entries to exceed the limit and trigger eviction.
    ...
    XCTAssertNotEqual(first, reloaded, @"Expected first entry to be evicted and reloaded after cost limit breach");
