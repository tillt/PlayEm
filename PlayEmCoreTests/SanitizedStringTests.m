...
{
    // Construct a UTF-16LE string for "ÐÑtest" and feed it through UTF-8
    // (mojibake markers present).
    ...
    // After fallback, we should no longer see the mojibake markers "Ã" or "Â" and
    // should get "ÐÑtest" or cleaned form.
...
{
    const uint8_t badBytes[] = {0x54, 0x6F, 0x77, 0x6E, 0x20, 0x4A, 0x6F, 0x6B, 0x65, 0x72, 0x20, 0x28, 0x50, 0x68, 0x69, 0x6C, 0x69,
                                0x70, 0x20, 0x42, 0x61, 0x64, 0x65, 0x72, 0x20, 0x26, 0x20, 0x4E, 0x69, 0x63, 0x6F, 0x6E, 0xC3, 0xA2,
                                0xC2, 0x88, 0xC2, 0x9A, 0xC3, 0x82, 0xC2, 0xAE, 0x20, 0x52, 0x65, 0x6D, 0x69, 0x78, 0x29};
    ...
    NSString* san = [bad sanitizedMetadataString];
    const uint8_t expectedBytes[] = {0x54, 0x6F, 0x77, 0x6E, 0x20, 0x4A, 0x6F, 0x6B, 0x65, 0x72, 0x20, 0x28, 0x50, 0x68,
                                     0x69, 0x6C, 0x69, 0x70, 0x20, 0x42, 0x61, 0x64, 0x65, 0x72, 0x20, 0x26, 0x20, 0x4E,
                                     0x69, 0x63, 0x6F, 0x6E, 0xC3, 0xA8, 0x20, 0x52, 0x65, 0x6D, 0x69, 0x78, 0x29};
...
{
    // Build the special Town Joker sample from bytes to avoid control chars in
    // source.
    const uint8_t townBadBytes[] = {0x54, 0x6F, 0x77, 0x6E, 0x20, 0x4A, 0x6F, 0x6B, 0x65, 0x72, 0x20, 0x28, 0x50, 0x68, 0x69, 0x6C, 0x69,
                                    0x70, 0x20, 0x42, 0x61, 0x64, 0x65, 0x72, 0x20, 0x26, 0x20, 0x4E, 0x69, 0x63, 0x6F, 0x6E, 0xC3, 0xA2,
                                    0xC2, 0x88, 0xC2, 0x9A, 0xC3, 0x82, 0xC2, 0xAE, 0x20, 0x52, 0x65, 0x6D, 0x69, 0x78, 0x29};
    NSString* townBad = [[NSString alloc] initWithBytes:townBadBytes length:sizeof(townBadBytes) encoding:NSUTF8StringEncoding];
    const uint8_t townExpectedBytes[] = {0x54, 0x6F, 0x77, 0x6E, 0x20, 0x4A, 0x6F, 0x6B, 0x65, 0x72, 0x20, 0x28, 0x50, 0x68,
                                         0x69, 0x6C, 0x69, 0x70, 0x20, 0x42, 0x61, 0x64, 0x65, 0x72, 0x20, 0x26, 0x20, 0x4E,
                                         0x69, 0x63, 0x6F, 0x6E, 0xC3, 0xA8, 0x20, 0x52, 0x65, 0x6D, 0x69, 0x78, 0x29};
    ...
    NSArray<NSDictionary<NSString*, NSString*>*>* samples = @[
        ...
        @{@"input" : @"FranÃ§ois becomes FranÃ§ois and EspaÃ±a and piÃ±a colada", @"expected" : @"François becomes François and España and piña colada"},
...
- (void)testLikelyCleanFastPathPerformance
{
    ...
    XCTSkip(@"Sanitizer micro-benchmark skipped unless ENABLE_SANITIZER_BENCH is "
            @"defined.");
#else
    NSString* clean = @"Hello World – Café";
    const NSUInteger iterations = 100000;

    // Manual timing so we can assert a per-call budget; keep XCT measure for
    // trend reporting.
