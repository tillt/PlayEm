...
// Test-only API declared here and implemented in
// TotalIdentificationController.m (Debug builds).
...
{
    // Use the recent detection output as hardcoded raw Shazam-style hits (two per
    // track for support≥2).
    ...
    void (^addTrack)(NSString*, NSString*, unsigned long long, NSTimeInterval, double) =
        ^(NSString* artist, NSString* title, unsigned long long frame, NSTimeInterval duration, double confidence) {
    ...
    XCTSkip(@"Synthetic hardcoded hit test is diagnostic only; golden-list "
            @"validation covers correctness.");
}
...
{
    XCTSkip(@"Raw single-hit diagnostic skipped; golden-list validation is "
            @"authoritative.");
}

// Parse “[Score] frame:… artist:… title:… duration:… score:… confidence:…”
...
- (void)testRefinementWithFullShazamLog
{
    ...
    for (TimedMediaMetaData* t in refined) {
        ...
        if ([title containsString:@"Seat 11"]) {
            hasSeat11 = YES;
        }
        if ([title containsString:@"Blackout"]) {
            hasBlackout = YES;
        }
        if ([title containsString:@"Eat You"] || [artist containsString:@"Eitan Reiter"]) {
            hasEatYou = YES;
        }
    ...
    // We are missing 4 golden tracks in the raw input; allow up to that many
    ...
    // Emit a quick diff vs. golden (excluding truly missing tracks) to guide
    // scoring tweaks.
    NSSet<NSString*>* presentAvailables =
        [NSSet setWithObjects:@"Seat 11", @"Slippery Road", @"Muted Glow", @"End of the Line", @"The Arboretum", @"Cosmo Acids", @"Masquerade",
                              @"Special Places", @"Hold Me", @"Go", @"Blackout", @"Calypso", @"Back In Trance", @"Eat You", @"What's Left", nil];
