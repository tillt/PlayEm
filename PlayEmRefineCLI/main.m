...
static NSArray<TimedMediaMetaData*>* ParseInputLog(NSString* path, unsigned long long* outMaxFrame)
{
    ...
    NSRegularExpression* re = [NSRegularExpression regularExpressionWithPattern:@"\\[(Input|Shazam)\\]\\s*frame:(\\d+)\\s*artist:([^\\r\\n]*)\\s*"
                                                                                @"title:([^\\r\\n]*)\\s*score:([0-9\\.]+)\\s*confidence:(null|[0-9\\."
                                                                                @"]+)"
                                                                        options:0
                                                                          error:nil];
...
int main(int argc, const char* argv[])
{
    @autoreleasepool {
        ...
        // Build stub sample context (~10 minutes beyond last frame to keep duration
        // math sane).
        ...
        for (NSUInteger i = 0; i < refined.count; i++) {
            ...
            NSLog(@"[Final] frame:%llu artist:%@ title:%@ duration:%.2fs score:%.3f "
                  @"confidence:%.3f",
                  t.frame.unsignedLongLongValue, t.meta.artist ?: @"", t.meta.title ?: @"", durationSeconds, t.score.doubleValue,
